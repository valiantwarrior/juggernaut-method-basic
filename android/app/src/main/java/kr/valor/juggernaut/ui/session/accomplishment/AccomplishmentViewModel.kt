...
class AccomplishmentViewModel @Inject constructor(
    ...
    val uiState: StateFlow<AccomplishmentUiState> = flow {
        ...
        val backDestinationToken: AccomplishmentDestinationToken = savedStateHandle[NAV_ARGS_ACCOMPLISHMENT_DESTINATION_TOKEN]!!
        val session = findSessionOneShotUseCase(sessionId)

        emit(AccomplishmentUiState.Result(session, backDestinationToken))
...
sealed class AccomplishmentUiState {
    ...
    data class Result(val session: Session, val destinationToken: AccomplishmentDestinationToken): AccomplishmentUiState()
}